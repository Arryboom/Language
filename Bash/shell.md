...
1、首先下载wondershaper的rpm安装包：
```
wondershaper-1.1a-7.noarch.rpm
```
2、安装wondershaper：
```
...
>if you are on openvz,you need host configured to support ntables or iptables.


#distinguish whether the system now using iptables or nftables.
...
A variant of this problem [was addressed recently in Kubernetes](https://github.com/kubernetes/kubernetes/issues/71305), so it’s worth looking at what was done there. (The variant is whether to use `iptables-legacy` or `iptables-nft` and their IPv6 variants to drive the host’s rules.)

The approach taken in Kubernetes is [to look at the number of lines output by the respective “save” commands](https://github.com/kubernetes/kubernetes/blob/master/build/debian-iptables/iptables-wrapper), `iptables-legacy-save` and `iptables-nft-save` (and their IPv6 variants). If the former produces ten lines or more of output, or produces more output than the latter, then it’s assumed that `iptables-legacy` should be used; otherwise, that `iptables-nft` should be used.
...
[root@ log]# service nftables status
...
● nftables.service - Netfilter Tables
   ...
   Active: inactive (dead)
     Docs: man:nft(8)
[root@ log]# service iptables status
...
2.  **符号>&是一个整体**，不可分开，分开后就不是上述含义了。  
    ...
    如果是尝试过，你就知道**2>1的写法其实是将标准错误输出重定向到名为"1"的文件里去了**
3.  写成**2&>1**也是不可以的
...
参考：  
[https://unix.stackexchange.com/questions/89386/what-is-symbol-and-in-unix-linux](https://unix.stackexchange.com/questions/89386/what-is-symbol-and-in-unix-linux)  
[https://superuser.com/questions/335396/what-is-the-difference-between-and-in-bash](https://superuser.com/questions/335396/what-is-the-difference-between-and-in-bash)


# centos 查看哪些包提供了缺失的命令
```
 yum provides "*/ping"
```
